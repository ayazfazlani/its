<section class="w-full p-6">
    ...
    <dialog id="user-modal" class="modal" @if ($showModal) open @endif>
        <form method="dialog" class="modal-box w-full max-w-2xl" wire:submit.prevent="saveUser">
            ...
            @if ($isEdit)
                <div class="alert alert-info mb-4">You can only assign roles to the user. Name, email, and password cannot be changed here.</div>
            @endif
            <div class="mb-4">
                <label class="label">Name</label>
                <input type="text" wire:model.defer="name" class="input input-bordered w-full" placeholder="Name" @if($isEdit) disabled @endif />
                @error('name')
            ...
            <div class="mb-4">
                ...
                <input type="email" wire:model.defer="email" class="input input-bordered w-full" placeholder="Email" @if($isEdit) disabled @endif />
                @error('email')
                    <span class="text-red-500 text-xs">{{ $message }}</span>
                @enderror
            </div>
            @if (!$isEdit)
                <div class="mb-4">
                    ...
                    <input type="password" wire:model.defer="password" class="input input-bordered w-full" placeholder="Password" />
                    @error('password')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <input type="password" wire:model.defer="password_confirmation" class="input input-bordered w-full" placeholder="Confirm Password" />
                </div>
            @endif
            <div class="mb-4">
            ...
            <div class="modal-action flex gap-2">
                ...
                <button type="submit" class="btn btn-primary">{{ $isEdit ? 'Update Roles' : 'Create' }}</button>
            </div>
        </form>
    </dialog>
