...
class PaymentUnclear extends Component
{
    ...
    protected function reloadData()
    {
        ...
        if ($isAdminOrManagerOrSupport) {
            $this->advertisements = Marketing::with(['employee.user'])->where('payment_status', 'uncleared')->get();
            $this->employees = Employee::with('user')->get();
        } else {
            $employee = Employee::with('user')->where('user_id', Auth::id())->first();
            $this->advertisements = Marketing::with(['employee.user'])->where('payment_status', 'uncleared')
    ...
    public function save()
    {
        ...
        $this->validate([
            ...
            'status' => 'required|in:active,pause,inActive,clientLeft,uncleared',
    ...
    {
        return view('livewire.pages.cstmr-sprt.payment-unclear', [
